...
class Conv2dNoSubsampling(BaseSubsampling):
    """Convolutional 2D without temporal subsampling.
    ...
    def __init__(
        ...
        """Construct a Conv2dNoSubsampling object."""
        super().__init__()
    ...
    def forward(
        ...
        """Pass-through without temporal subsampling.
        ...
        Returns:
            torch.Tensor: Tensor with same temporal length (#batch, time, odim).
    ...
    def __init__(
        ...
        self.conv = torch.nn.Sequential(
            ...
            torch.nn.ConvTranspose2d(
                odim,
                odim,
                kernel_size=(self.expansion_factor, 1),
                stride=(self.expansion_factor, 1),
                padding=(0, 0),
            ),
        )
        self.out = torch.nn.Sequential(
            torch.nn.Linear(odim * idim, odim), torch.nn.Dropout(dropout_rate)
        )
        self.pos_enc = pos_enc_class
        self.subsampling_rate = 1
        self.right_context = 0
...
class ConformerEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        # print(f"before conformer embedd of input, shape is {xs.shape}")
        xs, pos_emb, masks = self.embed(xs, masks)
        # print(f"after conformer embedd of input, shape is {xs.shape}")
        ...
        # print(f"after conformer encoder, shape is {xs.shape}")
        if self.normalize_before:
